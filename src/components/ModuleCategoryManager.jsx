...
// Même modèle que CategoryManager.jsx (documents), généralisé aux catégories réutilisables par
// plusieurs modules (voir routes/moduleCategories.js) — un seul composant paramétré par
// resourceType plutôt qu'une copie par module (CAPA, réclamations, QQOQCCP, fournisseurs,
// formations, revues de direction).
export default function ModuleCategoryManager({ resourceType, isAdmin }) {
  ...
  async function loadCategories() {
    ...
    setError('');
    try {
      const { data } = await api.get('/module-categories', { params: { resource_type: resourceType } });
      setCategories(data);
    } catch {
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (editingId === 'new') {
        const { data } = await api.post('/module-categories', { ...form, resource_type: resourceType });
        setCategories((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
      } else {
        const { data } = await api.put(`/module-categories/${editingId}`, form);
