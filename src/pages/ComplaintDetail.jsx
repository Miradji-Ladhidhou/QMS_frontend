...
function CreateCapaFromComplaintModal({ complaintId, complaint, users, services, priorityDelays, onClose, onCreated }) {
  const [form, setForm] = useState({
    title: '',
    service_id: '',
    priority: 'medium',
  ...
  async function handleSubmit(event) {
    ...
    let response;
    try {
      response = await api.post(`/complaints/${complaintId}/create-capa`, payload);
    } catch (err) {
